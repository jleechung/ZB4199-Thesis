...
First, we examine results on the RNA sequin spike-ins (n=41 transcripts). For each sample, we computed the SCC, NRMSE and MRD on the abundance estimates returned by each method (). 

\begin{figure}
    \centering
    \includegraphics{figures/sequin-metrics.png}
    \caption[SCC, NRMSE and MRD on sequencing spike-ins in SG-NEx data]{SCC, NRMSE and MRD on sequencing spike-ins in SG-NEx data. }
    \label{fig:my_label}
\end{figure}