...
To model real data, we develop an empirical read length-isoform agreement model based on the degradation curves derived from Section \ref{sec:rld}. Let $X$ be a random variable denoting the length of degraded reads, and $F(x)=P(X\leq x)$ be the corresponding empirical cumulative distribution function on $X$. Then, we have

\begin{equation}\label{eq:exact-rlia}
    p(\ell_i\mid d_j, z_{ij}=1) = 
    \begin{cases}
        \dfrac{F(\ell_i+\delta) - F(\ell_i-\delta)}{F(\mathrm{len}(j))} & \textrm{if } \ell_i<\min(\mathrm{len}(j), \ell_\mathrm{max})\\
        1-F(\mathrm{len}(j)) & \textrm{if } \ell_i=\mathrm{len}(j)
    \end{cases}
\end{equation}

Recall again from section \ref{sec:rld} that the probability of observing a read of length $\ell_i$ or greater is $1-F(\ell_i)$. Thus, for a transcript of length $\mathrm{len}(j)$, the probability of observing a full length read is simply $1-F(\mathrm{len}(j))$. The corresponding probability of observing degraded reads is $F(\mathrm{len}(j))$. To compute the probability of observing a point given a continuous function $F$, we take limtis of $\delta\rightarrow0$ of
\begin{equation}
\begin{split}
    P(X=\ell_i) &=P(X<\ell_i+\delta)-P(X<\ell_i-\delta)\\
    &=F(\ell_i+\delta)-F(\ell_i-\delta)
\end{split}
\end{equation}
We normalize this probability by the probability of observing degraded reads, arriving at the quantity
\begin{equation}
    \dfrac{F(\ell_i+\delta)-F(\ell_i-\delta)}{F(\mathrm{len}(j))}
\end{equation}
