...
\begin{equation}\label{eq:emp-rlia}
    p(\ell_i\mid d_j, z_{ij}=1) = 
    \begin{cases}
        \lim\limits_{\delta\rightarrow0}\dfrac{F(\ell_i+\delta) - F(\ell_i-\delta)}{F(\mathrm{len}(j))} & \textrm{if } \ell_i<\min(\mathrm{len}(j), \ell_\mathrm{max})\\
        1-F(\mathrm{len}(j)) & \textrm{if } \ell_i=\mathrm{len}(j)
    \end{cases}
\end{equation}

Recall again from section \ref{sec:rld} that the probability of observing a read of length $\ell_i$ or greater is $S(\ell_i)=1-F(\ell_i)$. Thus, for a transcript of length $\mathrm{len}(j)$, the probability of observing a full length read is simply $1-F(\mathrm{len}(j))$. The corresponding probability of observing degraded reads is $F(\mathrm{len}(j))$. To compute the probability of observing a point given a empirical continuous density, we take limits:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
We normalize this quantity by the probability of observing degraded reads, arriving at the quantity
\begin{equation}
