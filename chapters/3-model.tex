...
\end{equation}
For example, a $\mathbb{E}[d]=0.2$ implies that $\ell_\mathrm{max}=5$ kb. This is an unrealistic assumption, given that \gls{ont} devices are capable of sequencing reads upwards of $10$ kb in length. Nevertheless, this simplifying assumption enables us to test a simpler read length-isoform agreement model on simulated datasets. We develop a more flexible model allowing for variable degradation in the following section.  
...
\subsection{Likelihood formulation}\label{likelihood-formulation}

The marginal distribution of $r_i$ is obtained by summing the joint distribution (Eq. \ref{eq:joint-dist}) over all $j$ possible stats of $z_i$. From Eqs. \ref{eq:latent-dist}, \ref{eq:cond-dist} and \ref{eq:read-iso-agreement}, we obtain
\begin{equation}
...
\end{equation}
It is difficult to maximize Eq. \ref{eq:lik} with respect to $\bm{\theta}$ because the log acts on the sum. Assuming the latent variables $\bm{Z}$ are observed, the likelihood will decompose. We can write the complete data likelihood as
\begin{equation}
...
We describe two strategies to infer the parameters $\bm{\theta}$ that maximize the likelihood of the observed data: expectation maximization and variational inference.  
...
\end{equation}
Taking the derivative with respect to $\theta_j$ and setting to $0$ we obtain
\begin{equation}
...
\end{equation}
From Eqs. \ref{eq:derivative} and \ref{eq:lambda} we obtain
\begin{equation}
...
\begin{enumerate}
    ...
    \item Evaluate the log-likelihood in Eq. \ref{eq:log-lik}
    \item Check for convergence in the parameters or log-likelihood. If convergence criterion is not satisfied, return to step 2.
\end{enumerate}
